...
import {checkQuality, checkDynamicResult, checkStaticResult, distanceInNm, configureQuality} from './check-quality';
import {defaultOptions} from './ais-decoder';
import type {AisSuccessResult} from './definitions';

function makeResult(fields: Partial<AisSuccessResult>): AisSuccessResult {
    return {status: 'decoded', mmsi: 0, channel: 'A', payload: new Uint8Array(), ...fields};
}
...
describe('check-quality', () => {
    ...
    describe('checkDynamicResult', () => {
        ...
        it('should skip the first dynamic transmission', () => {
            expect(() => checkDynamicResult(makeResult({mmsi: 100000001, lon: 1, lat: 1})))
        ...
        it('should accept the third dynamic transmission', () => {
            expect(checkDynamicResult(makeResult({mmsi: 100000001, lon: 1, lat: 1}))).toBe(true);
        });
        ...
        it('should track each mmsi independently', () => {
            expect(() => checkDynamicResult(makeResult({mmsi: 100000002, lon: 1, lat: 1})))
                .toThrow('Skipping initial dynamic transmission #1');

            expect(() => checkDynamicResult(makeResult({mmsi: 100000003, lon: 1, lat: 1})))
                .toThrow('Skipping initial dynamic transmission #1');

            expect(() => checkDynamicResult(makeResult({mmsi: 100000002, lon: 1, lat: 1})))
                .toThrow('Skipping initial dynamic transmission #2');
            expect(checkDynamicResult(makeResult({mmsi: 100000002, lon: 1, lat: 1}))).toBe(true);
        });
    });
    ...
    describe('checkDynamicResult reset timeout based on SOG', () => {
        ...
        it('should reset after 10 minutes for moving vessel (sog >= 1)', () => {
            ...
            const pos = makeResult({mmsi, lon: 10, lat: 50, sog: 5});
        ...
        it('should reset after 30 minutes for stopped vessel (sog < 1)', () => {
            ...
            const pos = makeResult({mmsi, lon: 10, lat: 50, sog: 0});
        ...
        it('should not reset within 30 minutes for stopped vessel', () => {
            ...
            const pos = makeResult({mmsi, lon: 10, lat: 50, sog: 0.5});
        ...
        it('should use 10-minute timeout when sog is undefined', () => {
            ...
            const pos = makeResult({mmsi, lon: 10, lat: 50});
    ...
    describe('checkStaticResult', () => {
        ...
        it('should skip the first static transmission', () => {
            expect(() => checkStaticResult(makeResult({mmsi: 100000001})))
                .toThrow('Skipping initial static transmission #1');
        });

        it('should accept the second static transmission', () => {
            expect(checkStaticResult(makeResult({mmsi: 100000001}))).toBe(true);
        });
        ...
        it('should track each mmsi independently', () => {
            expect(() => checkStaticResult(makeResult({mmsi: 100000002})))
                .toThrow('Skipping initial static transmission #1');

            expect(checkStaticResult(makeResult({mmsi: 100000002}))).toBe(true);

            expect(() => checkStaticResult(makeResult({mmsi: 100000003})))
                .toThrow('Skipping initial static transmission #1');
        });
    });
    ...
    describe('checkQuality', () => {
        it('should route to dynamic check when result has lon', () => {
            expect(() => checkQuality(makeResult({mmsi: 300000001, lon: 1.5, lat: 1})))
                .toThrow('Skipping initial dynamic transmission #1');
        });

        it('should route to static check when result has no lon', () => {
            expect(() => checkQuality(makeResult({mmsi: 300000001, name: 'VESSEL'})))
                .toThrow('Skipping initial static transmission #1');
        });

        it('should route to static check when lon is not a number', () => {
            expect(() => checkQuality(makeResult({mmsi: 300000002, lon: undefined as any})))
                .toThrow('Skipping initial static transmission #1');
        });
    });
    ...
    describe('checkDynamicResult distance check', () => {
        it('should accept when position is within 1 nm', () => {
            ...
            const pos = makeResult({mmsi, lon: 144.0, lat: -38.0});
        ...
        it('should throw when position jumps more than 1 nm', () => {
            ...
            const farAway = makeResult({mmsi, lon: 12.0, lat: 52.0});
        ...
        it('should not check distance on the first accepted transmission', () => {
            ...
            const pos = makeResult({mmsi, lon: 100.0, lat: 20.0});
    ...
    describe('checkDynamicResult SAR aircraft exemption', () => {
        it('should skip distance check for mtype 9 (SAR aircraft)', () => {
            ...
            const pos = makeResult({mmsi, mtype: 9, lon: 10.0, lat: 50.0});
            ...
            const farAway = makeResult({mmsi, mtype: 9, lon: 15.0, lat: 55.0});
            expect(checkDynamicResult(farAway)).toBe(true);
        });

        it('should still check distance for non-SAR mtype', () => {
            const mmsi = 500000021;
            const pos = makeResult({mmsi, mtype: 1, lon: 10.0, lat: 50.0});
            ...
            const farAway = makeResult({mmsi, mtype: 1, lon: 12.0, lat: 52.0});
            expect(() => checkDynamicResult(farAway)).toThrow('position jumped');
        });
    });
    ...
    describe('configureQuality', () => {
        it('should allow setting requiredDynamic to 0 to disable dynamic checks', () => {
            ...
            const result = checkDynamicResult(makeResult({mmsi: 900000001, lon: 1, lat: 1}));
        ...
        it('should allow setting requiredStatic to 0 to disable static checks', () => {
            ...
            const result = checkStaticResult(makeResult({mmsi: 900000002}));
            expect(result).toBe(true);
            // restore default
        ...
        it('should allow changing maxDistanceNm', () => {
            ...
            const far = makeResult({mmsi, lon: 11.0, lat: 51.0});
